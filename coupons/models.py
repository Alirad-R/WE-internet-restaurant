    ...
    def is_valid(self, user=None, cart_value=None, cart_items=None, order_items=None):
        ...
        try:
            ...
            if user:
                ...
                if not self._validate_customer_tier(user):
                    return False
                
                # Validate order history
                if not self._validate_order_history(user):
                    return False
                
                # Validate usage interval
                if not self._validate_usage_interval(user):
                    return False
            ...
            if order_items:
                ...
                if not self._validate_category_requirements(order_items):
                    return False
                
                # Check for excluded items
                if self._has_excluded_items(order_items):
                    return False
                
                # Check discount combinations
                if not self._validate_discount_combinations(order_items):
                    return False
            
            # Time and seasonal restrictions
            if not self._validate_time_restrictions():
                return False
            
            if not self._validate_seasonal_restrictions():
                return False
